...
test_that('GC-XCMS data returned correctly',{
  expect_s4_class(GC_processed_data,'MetaboProfile')
})

test_that('Processed GC-XCMS data can be extracted',{
  expect_s3_class(processedData(GC_processed_data),'tbl_df')
})

test_that('GC-XCMS peak info can be extracted',{
  expect_s3_class(peakInfo(GC_processed_data),'tbl_df')
})

test_that('The processing object can be extracted for GC-XCMS data',{
  expect_s4_class(extractProcObject(GC_processed_data),'XCMSnExp')
})

test_that('Chromatogram can be plotted for GC-XCMS data',{
  expect_s3_class(plotChromatogram(GC_processed_data),'ggplot')
})

test_that('TIC can be plotted for GC-XCMS data',{
